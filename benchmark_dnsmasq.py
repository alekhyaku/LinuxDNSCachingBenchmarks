...
DEFAULT_LOCAL_RESOLVER = "127.0.0.1"  # systemd-resolved stub resolver (change to "127.0.0.1" for dnsmasq)
...
def flush_dnsmasq_cache():
    
    if MODE in ("encrypted", "fallback"):
        try:
            subprocess.run(["sudo", "systemctl", "restart", "stubby"], check=True)
            print("[*] stubby restarted (TLS session flushed).")
        except Exception as e:
            print(f"[!] Failed to restart stubby: {e}")
        
    try:
        subprocess.run(["sudo", "systemctl", "restart", "dnsmasq"], check=True)
        print("dnsmasq cache flushed via service restart.")
    except subprocess.CalledProcessError as e:
        print("Failed to flush dnsmasq cache:", e)
    time.sleep(1)
...
def multi_domain_cache_test(domains, iterations=3):
    ...
    """
    print(f"\nRunning multi-domain cache fill test... domains: {domains}")
    flush_dnsmasq_cache()
    cold_latencies = []
    warm_latencies = []
...
def eviction_pressure_test(domains, cache_size_guess=1000):
    ...
    if cache_size_guess < 10000:
        requery_size = cache_size_guess

    print("  Priming complete. Re-querying the first 10,000 domains to assess eviction...")
    requery_domains = pressure_domains[:requery_size]
    hit_times = []
...
def memory_cpu_utilization_test(duration):
    ...
    for i in range(duration):
        try:
            ...
            print("dnsmasq process ended unexpectedly.")
            break
...
def fallback_test():
    """
    Simulate DNS-over-TLS failure for dnsmasq+Stubby setup:
      1) Stop Stubby (so the encrypted upstream is dead)
      2) Restart dnsmasq (flush + re-read config)
      3) Run a query (dnsmasq should fall back to 8.8.8.8 over UDP)
      4) Restart Stubby and dnsmasq to restore encrypted path
    """
    print("\nRunning dnsmasq fallback test (stop Stubby to force UDP fallback)…")

    try:
        # 1) Stop Stubby so the TLS upstream at 127.0.0.1:8053 goes away
        print("[*] Stopping stubby (encrypted upstream)…")
        subprocess.run(["sudo", "systemctl", "stop", "stubby"], check=True)

        # 2) Restart dnsmasq to clear cache and pick up that stubby is gone
        print("[*] Restarting dnsmasq (cache flush + re-read servers)…")
        subprocess.run(["sudo", "systemctl", "restart", "dnsmasq"], check=True)
        time.sleep(2)

        # 3) Issue a DNS query via dnsmasq – should go to 8.8.8.8 over UDP
        qt, output = run_dig()
        print("\nTest query result when Stubby is stopped (expect UDP to 8.8.8.8):")
        print(output)

        # Optionally, show that fallback happened by inspecting system logs:
        # subprocess.run(["journalctl", "-u", "dnsmasq", "--since", "1 minute ago"], check=True)
    except Exception as e:
        print(f"[!] Error during dnsmasq fallback test: {e}")
    finally:
        # 4a) Restart Stubby so the encrypted upstream comes back
        print("[*] Restarting stubby to restore encryption…")
        subprocess.run(["sudo", "systemctl", "start", "stubby"], check=True)
        time.sleep(1)

        # 4b) Restart dnsmasq again to re‑enable forwarding to Stubby
        print("[*] Restarting dnsmasq (re‑enable encrypted upstream)…")
        subprocess.run(["sudo", "systemctl", "restart", "dnsmasq"], check=True)
        time.sleep(1)

        print("[*] dnsmasq fallback test complete.")
