...
DEFAULT_LOCAL_RESOLVER = "127.0.0.1"  # bind9 stub resolver
...
def memory_cpu_utilization_test(duration):
    """Monitor memory and CPU usage of bind9 over a specified duration (in seconds)."""
    print(f"\nRunning memory and CPU utilization test for {duration} seconds...")
    proc = None
    for p in psutil.process_iter(['name']):
        try:
            if p.info['name'] == "named":
                proc = p
                break
        except (psutil.NoSuchProcess, psutil.AccessDenied):
            continue
    if not proc:
        print("bind9 process not found!")
        return
    mem_samples = []
    cpu_samples = []
    for i in range(duration):
        try:
            ...
            print("bind9 process ended unexpectedly.")
            break
...
def fallback_test():
    """
    Simulate DNS-over-TLS failure for BIND + Stubby setup:
      1) Stop Stubby (so the encrypted upstream is dead)
      2) Restart BIND (flush + re-read config)
      3) Run a query (BIND should fall back to 8.8.8.8 over UDP)
      4) Restart Stubby and BIND to restore encrypted path
    """
    print("\nRunning BIND fallback test (stop Stubby to force UDP fallback)…")
    ...
    try:
        ...
        # 2) Restart BIND to clear any forwarding state and reload configs
        print("[*] Restarting BIND (cache flush + re-read forwarders)…")
        subprocess.run(["sudo", "systemctl", "restart", "bind9"], check=True)
        time.sleep(2)

        # 3) Issue a DNS query via BIND – should go to 8.8.8.8 over UDP
        qt, output = run_dig()
        print("\nTest query result when Stubby is stopped (expect UDP fallback to 8.8.8.8):")
        print(output)

    except Exception as e:
        print(f"[!] Error during BIND fallback test: {e}")
    finally:
        # 4a) Restart Stubby to bring back the secure upstream
        print("[*] Restarting stubby to restore encryption…")
        subprocess.run(["sudo", "systemctl", "start", "stubby"], check=True)
        time.sleep(1)

        # 4b) Restart BIND again to pick up that Stubby is back
        print("[*] Restarting BIND (re-enable encrypted upstream)…")
        subprocess.run(["sudo", "systemctl", "restart", "bind9"], check=True)
        time.sleep(1)

        print("[*] BIND fallback test complete.")
...
def main():
    parser = argparse.ArgumentParser(
        description="Rigorous Benchmarking for bind9 performance testing."
    )
